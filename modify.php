...
//			}else{
			if(image==null){
//				$('#head').attr('src','Heads/default.jpg');
				if(gender==1){
					$('#head').attr('src','Heads/header_man.png');
				}else if(gender==0){
					$('#head').attr('src','Heads/header_woman.png');
...
//						});
						...
						var txt='Please check the modified information';
				...
				var flog = checkFormmodify1();
					if(flog!=false){
						...
						$.ajaxFileUpload({
							...
							dataType: "json",//返回数据的类型
							data:{
								url:"http://101.37.100.209:9001/loginappDate/modify",
								params:"firstName="+$('#firstName').val()+"&password="+$('#password').val()+"&middleName="+$('#middleName').val()+"&lastName="+$('#lastName').val()+"&birth="+$("#appDate").val()+"&gender="+$('#gender').val()+"&mphone="+$('#phone').val()+"&address="+$('#address').val()+"&email="+$('#email').val()+'&head='+$('#path').val(),
								token:token_t.substring(1,token_t.length-1)
							},//一同上传的数据
							success: function (data) {
